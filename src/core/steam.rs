...
/// **last argument that ends with `.exe`** with the full path to `chosen_exe`,
...
///
/// Matching the last `.exe` argument (case-insensitive) rather than a specific
/// filename means the wrapper works regardless of what Steam's own launch
/// configuration uses as its default executable (e.g. the game launcher vs the
/// game binary itself).
fn write_exe_override_script(
    ...
    // Replace the last argument ending in .exe (case-insensitive) with the
    // chosen exe path.  This is robust against Steam using a launcher (e.g.
    // SkyrimLauncher.exe) as the default Proton target — we always end up
    // running the exe the user actually selected.
    let script = format!(
        "#!/usr/bin/env python3\n\
         ...
         \tif args[i].lower().endswith('.exe'):\n\
         \t\targs[i] = CHOSEN\n\
         \t\treplaced = True\n\
         \t\tbreak\n\
         if not replaced:\n\
         \tprint('linkmm: no .exe argument found in Steam Proton command; cannot override executable', file=sys.stderr)\n\
         \tsys.exit(1)\n\
         if args:\n\
...
///
/// A small Python 3 wrapper script is written to `/tmp` and registered as
/// the game's `LaunchOptions` in Steam's `localconfig.vdf`.  The wrapper
/// intercepts Steam's Proton invocation and substitutes the chosen executable
/// for whatever Steam would run by default (often a launcher binary), so that
/// Proton runs the binary the user selected while Steam still tracks the
...
pub fn launch_game_executable(
    ...
    // For every exe (primary game binary, launcher, or script extender) write
    // a wrapper that replaces the last .exe argument in Steam's Proton command
    // with the chosen path.  This ensures the selected binary runs regardless
    // of what Steam's own default launch configuration points to (which is
    // often a game launcher rather than the game or script-extender binary).
    let script = write_exe_override_script(app_id, &exe_path)?;
...
mod tests {
    ...
    fn launch_game_executable_script_extender_fails_without_localconfig() {
        ...
        // localconfig.vdf), launching any exe should fail with a
    ...
    #[test]
    fn launch_game_executable_primary_exe_fails_without_localconfig() {
        // The primary exe now also requires localconfig.vdf (it uses the same
        // wrapper mechanism as script extenders so the correct exe runs, not
        // the Steam-default launcher).
        let tmp = tempfile::tempdir().expect("tempdir");
        std::fs::write(tmp.path().join("SkyrimSE.exe"), b"fake").unwrap();
        let game = Game::new(GameKind::SkyrimSE, tmp.path().to_path_buf());
        let result = launch_game_executable(&game, "SkyrimSE.exe");
        assert!(result.is_err());
        let err = result.unwrap_err();
